...
namespace Cash.Repository.DataAccess
{
    public class ProcessRepository : ModelRepository<Process>, IProcessRepository
    {
        ...
        public async Task<List<Process>> GetListWithAccountByReceiverAsync(int userId) => await _appDbContext.Processes.Where(x => x.ReceiverId == userId).Include(x => x.SenderPrc).ThenInclude(x => x.User).Where(x => x.ReceiverPrc.UserId == userId).ToListAsync();
        public async Task<List<Process>> GetListWithAccountBySenderAsync(int userId) => await _appDbContext.Processes.Include(x => x.ReceiverPrc).ThenInclude(x => x.User).Where(x => x.SenderPrc.UserId == userId).ToListAsync();
    }
}
